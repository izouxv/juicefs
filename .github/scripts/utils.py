...
def flush_meta(meta_url):
    ...
    if meta_url.startswith('sqlite3://'):
    ...
    elif meta_url.startswith('bbolt://'):
        path = meta_url[len('bbolt://'):]
        if os.path.isdir(path):
            shutil.rmtree(path)
            print(f'remove bbolt dir {path} succeed')
        ...
        run_cmd(
            '''fdbcli -C /home/runner/fdb.cluster --exec "writemode on ; clearrange '' \xFF"''')
    else:
...
def create_mysql_db(meta_url):
    ...
    if ':' in host_port:
        ...
        port = '3306'
    run_cmd(
        f'mysql -u{user} {password} -h {host} -P {port} -e "drop database if exists {db_name}; create database {db_name};"')
...
def clear_storage(storage, bucket, volume):
    ...
    if storage == 'file':
        ...
        url = urlparse(bucket)
        c = Minio('localhost:9000', access_key='minioadmin',
        ...
        while c.bucket_exists(bucket_name) and list(c.list_objects(bucket_name)):
            ...
            result = run_cmd(
                f'mc rm --recursive --force  myminio/{bucket_name}')
            if result != 0:
        ...
        run_cmd(
            f'mysql -uroot -proot -h localhost -P 3306 -e "drop database if exists {db_name};create database {db_name};"')
...
def get_stage_blocks(filesystem):
    try:
        ps = subprocess.Popen(
            ('cat', f'{filesystem}/.stats'), stdout=subprocess.PIPE)
        output = subprocess.check_output(
            ('grep', 'juicefs_staging_blocks'), stdin=ps.stdout)
        ps.wait()
...
def mdtest(filesystem, meta_url):
    ...
    os.chdir(filesystem)
    run_jfs_cmd(
        f'{juicefs_new} mdtest {meta_url} mdtest --dirs 5 --depth 2 --files 5 --threads 5 --write 8192'.split())
    os.chdir(cwd)
...
def run_jfs_cmd(options):
    ...
    try:
        output = subprocess.run(
            options, check=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    except subprocess.CalledProcessError as e:
        print(
            f'<FATAL>: subprocess run error, return code: {e.returncode} , error message: {e.output.decode()}')
        raise Exception('subprocess run error')
    print(
        f'run_jfs_cmd return code: {output.returncode}, output: {output.stdout.decode()}')
...
def run_cmd(command):
    ...
    try:
        output = subprocess.run(
            command.split(), check=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    except subprocess.CalledProcessError as e:
        print(
            f'<FATAL>: subprocess run error, return code: {e.returncode} , error message: {e.output.decode()}')
        return e.returncode
...
def get_storage(juicefs, meta_url):
    output = subprocess.run([juicefs, 'status', meta_url],
...
if __name__ == "__main__":
    run_jfs_cmd(['./juicefs-1.1.0-dev', 'rmr',
                '/tmp/sync-test/file_to_rmr', '--debug'])
